...
// где лежит основа? здесь или в Десктопе?
	...
	var Chat = function () {
        ...
        this.init = function () {
			...
			*/
			...
			if (isNodeWebkit) {
				global.window.nwDispatcher.requireNwGui().Window.get().on("blur",  function(){ 
					self.windowStatus = "hide"; 
				});
				global.window.nwDispatcher.requireNwGui().Window.get().on("focus", function(){ 
					...
					if(e == 'XMPP authentication failure') {
						...
						if(self.myIgnoreList.indexOf(from) == -1) {
								...
								if(body.size() && proc_body) {
									...
									if(self.myDialogList.indexOf(from) == -1) {
		...
		this.generateNotifyHtml5 = function(jid, title, text) {
			if (Notification) {
				if (Notification.permission !== "granted") {
					...
					notification.onclose = function () {console.log(n);
		...
		this.closeNotify = function(jid) {
			//console.log('close' + jid);
			if(isNodeWebkit) {
				if(typeof self.notifyAppList[jid] != 'undefined') {
					self.notifyAppList[jid].close();
				}
			} else {
		...
		this.generateNotify = function(type, jid, a) {
			...
			var style = '* {margin: 0;padding: 0;-webkit-box-sizing: border-box;-moz-box-sizing: border-box;box-sizing: border-box;}'+
										'body {font-size: 12px;line-height:18px;font-family:Arial, Verdana, sans-serif;width: 100%;height: 100%;background:#FFF;color:#404040;width:100%;}'+
										'.notifylist {}'+
										'.notifylist .notify{width:500px;background:#f2f1f1;border-top:4px solid #e2e0e0;padding:15px 20px 20px;}'+
										'.notifylist .notify .name{font-size:18px;color:#404040;line-height:22px;margin:0 0 15px 0;}'+
										'.notifylist .notify .text_top{font-size:15px;color:#404040;line-height:20px;margin:0 0 10px 0;}'+
										'.notifylist .notify .text{margin:0 0 20px 0;}'+
										'.notifylist .notify .text textarea{height:90px;width:100%;padding:10px;resize:none;overflow:auto;}'+
										'.notifylist .notify .fastanswer_block{margin:0 0 20px 0;}'+
										'.notifylist .notify .fastanswer_block textarea{height:90px;width:100%;padding:10px;resize:none;overflow:auto;}'+
										'.notifylist .notify .btns{text-align:center;}'+
										'.notifylist .notify .btns button{display:inline-block;height:38px;width:145px;line-height:38px;margin:0 10px 0 0;}'+
										'.notifylist .notify .btns button:last-child{margin:0;}'+
										'input:-webkit-autofill {-webkit-box-shadow: 0 0 0px 1000px white inset;}'+
										'input, textarea, select, #msgwnd, .textarea {background: none repeat scroll 0 0 #fff;border: medium none;border-radius: 5px;box-shadow: 0 3px 1px 0 #e5e5e5 inset;overflow: hidden;-webkit-box-sizing: border-box;-moz-box-sizing: border-box;box-sizing: border-box;font-size:12px;color:#595959;outline:none;}'+
										'.btn {background: none repeat scroll 0 0 #fff;border: medium none;border-radius: 5px;box-shadow: 0 3px 1px 0 #e5e5e5 inset;overflow: hidden;font-size:12px;	color:#595959;}'+
										'.submit {background:#7ab51d;box-shadow:none;color:#FFF;font-size:14px;cursor:pointer;}'+
										'.submit:hover {background:#669933;}'+
										'.cancel {background:#c7c5c5;box-shadow:none;color:#404040;font-size:14px;cursor:pointer;}'+
										'.gray {background:#c7c5c5;box-shadow:none;color:#404040;font-size:14px;cursor:pointer;}'+
										'.gray_sv {background:#dbdbdb;box-shadow:none;color:#404040;font-size:14px;cursor:pointer;}'+
										'.gray_sv_light {background:#e8e8e8;box-shadow:none;color:#808080;font-size:14px;cursor:pointer;}'+
										'.disabled_submit {background:#d4d1d1;box-shadow:none;color:#FFF;font-size:14px;cursor:default;}'+
										'.disabled_submit:hover {background:#d4d1d1;}';
					...
					if(showMainNotify) {
						if(isNodeWebkit) {
							if(typeof self.notifyAppList[jid] == 'undefined') {
									...
									var htmlText = '<div id="notification" class="notifylist">'+
										'<div class="notify" data-jid="'+jid+'"><div class="name">Новый диалог</div><div class="text_top">'+u.client_name+'</div><div class="text"><textarea>'+a+'</textarea></div><div class="btns"><button class="chat btn submit" onclick="window.emit(\'chat.click\')">Ответить</button><button class="answer btn gray_sv" onclick="window.emit(\'answer.click\')">Быстрый ответ</button><button class="ignore btn cancel" onclick="window.emit(\'ignore.click\')">Игнорировать</button></div></div>'+
									'</div>';
							...
							if(!$('.notifylist').find('.notify[data-jid="'+jid+'"]').size()) {
								var u = self.userExtraData[jid];
								$('.notifylist').append('<div class="notify" data-jid="'+jid+'"><div class="name">Новый диалог</div><div class="text_top">'+u.client_name+'</div><div class="text"><textarea>'+a+'</textarea></div><div class="btns"><button class="chat btn submit">Ответить</button><button class="answer btn gray_sv">Быстрый ответ</button><button class="ignore btn cancel">Игнорировать</button></div></div>');
					...
					if(showMainNotify) {
						if(isNodeWebkit) {
							...
							if(typeof self.notifyAppList[jid] == 'undefined') {
								var u = self.userExtraData[jid];
								var htmlText = '<div id="notification" class="notifylist">'+
									'<div class="notify" data-jid="'+jid+'"><div class="name">'+u.client_name+'</div><div class="text"><textarea>'+a+'</textarea></div><div class="btns"><button onclick="window.emit(\'chat.click\')" class="chat btn submit">К чату</button><button class="answer btn gray_sv" onclick="window.emit(\'answer.click\')">Быстрый ответ</button><button class="ignore btn cancel" onclick="window.emit(\'ignore.click\')">Игнорировать</button></div></div>'+
								'</div>';
								var notif = new DesktopNotification('Новое сообщение', {
									//icon: icon, 
									htmlBody: htmlText, 
							...
							if(!$('.notifylist').find('.notify[data-jid="'+jid+'"]').size()) {
								var u = self.userExtraData[jid];
								$('.notifylist').append('<div class="notify" data-jid="'+jid+'"><div class="name">'+u.client_name+'</div><div class="text"><textarea>'+a+'</textarea></div><div class="btns"><button class="chat btn submit">К чату</button><button class="answer btn gray_sv">Быстрый ответ</button><button class="ignore btn cancel">Игнорировать</button></div></div>');
								$('.notifylist').find('.notify[data-jid="'+jid+'"]').find('.chat').on('click', function() {
									if(!$('.notifylist').find('.notify[data-jid="'+jid+'"]').find('.fastanswer_block').size()) {
										self.actionThread(jid);
									} else {
					...
					if(showMainNotify) {
						if(isNodeWebkit) {
							if(typeof self.notifyAppList[jid] == 'undefined') {
									var u = self.userExtraData[jid];
									var htmlText = '<div id="notification" class="notifylist">'+
										'<div class="notify" data-jid="'+jid+'"><div class="name">'+u.client_name+'</div><div class="text">Вы не можете игнорировать этот диалог, т.к. вы единственный оставшийся оператор</div><div class="btns"><button class="chat btn submit" onclick="window.emit(\'chat.click\')">К чату</button><button class="answer btn gray_sv" onclick="window.emit(\'answer.click\')">Быстрый ответ</button></div></div>'+
									'</div>';
									var notif = new DesktopNotification('Новое сообщение', {
										//icon: icon, 
										htmlBody: htmlText, 
							...
							if(!$('.notifylist').find('.notify[data-jid="'+jid+'"]').size()) {
								var u = self.userExtraData[jid];
								$('.notifylist').append('<div class="notify" data-jid="'+jid+'"><div class="name">'+u.client_name+'</div><div class="text">Вы не можете игнорировать этот диалог, т.к. вы единственный оставшийся оператор</div><div class="btns"><button class="chat btn submit">К чату</button><button class="answer btn gray_sv">Быстрый ответ</button></div></div>');
								$('.notifylist').find('.notify[data-jid="'+jid+'"]').find('.chat').on('click', function() {
									if(!$('.notifylist').find('.notify[data-jid="'+jid+'"]').find('.fastanswer_block').size()) {
										self.acceptDialogSend(jid);
										self.actionThread(jid);
			...
			if(type == 'closeAllDialogs') {
					...
					$('.dialog_window_btn.submit').on('click', function() {
						self.myDialogList.forEach(function(item, i, arr) {
							...
							//self.setClientData(item, false, false, 'close');
						});
						self.setStatus('away');
